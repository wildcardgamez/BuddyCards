        ...
        if (event.getSource().getEntity() instanceof PlayerEntity) {
            ...
            if (entity instanceof ZombifiedPiglinEntity) {
                if (entity.isBaby() && Math.random() < ConfigManager.zombiePiglinChance.get()) {
                    drops.add(new ItemEntity(world, entity.getX(), entity.getY(), entity.getZ(), new ItemStack(RegistryHandler.PACK_NETHER.get(), 1)));
                }
            }
            else if (entity instanceof ZombieVillagerEntity && entity.isBaby()) {
                if (Math.random() < ConfigManager.zombieVillagerChance.get()) {
                    drops.add(new ItemEntity(world, entity.getX(), entity.getY(), entity.getZ(), new ItemStack(RegistryHandler.PACK_BASE.get(), 1)));
                }
            }
                ...
                if (Math.random() < ConfigManager.dragonChance.get()) {
                    drops.add(new ItemEntity(world, entity.getX(), entity.getY(), entity.getZ(), new ItemStack(RegistryHandler.PACK_END.get(),
                            (int) (Math.random() * ConfigManager.dragonMaxPacks.get()) + 1)));
                }
            }
            else if (entity instanceof WitherEntity) {
                if (Math.random() < ConfigManager.witherChance.get()) {
                    drops.add(new ItemEntity(world, entity.getX(), entity.getY(), entity.getZ(), new ItemStack(RegistryHandler.PACK_NETHER.get(),
